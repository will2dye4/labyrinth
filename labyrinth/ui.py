...
class MazeApp(tk.Frame):
    ...
    def __init__(self, master=None, width=10, height=10, validate_moves=True):
        ...
        self.pack()
        self.canvas = self.create_maze_grid()
        self.stats = self.create_stats_display()
        self.start_time = None
        self.end_time = None

    def create_maze_grid(self):
        width = self.CELL_WIDTH * self.maze.width + self.BORDER_OFFSET
        height = self.CELL_HEIGHT * self.maze.height + self.BORDER_OFFSET
        ...
        for row in range(self.maze.height):
            for column in range(self.maze.width):
                ...
                directions = {
                    ...
                    Direction.S: ((x0, y1, x1, y1), lambda r, c: r == self.maze.height - 1),
                    Direction.E: ((x1, y0, x1, y1), lambda r, c: c == self.maze.width - 1),
                ...
                for direction in directions:
                    ...
                    if direction not in cell.open_walls:
                        ...
                        canvas.create_line(*coordinates, width=width)
    ...
    def get_selected_cell_coordinates(self, event):
        row = max(0, min(event.y // self.CELL_HEIGHT, self.maze.height - 1))
        column = max(0, min(event.x // self.CELL_WIDTH, self.maze.width - 1))
        return row, column
    ...
    def select_cell(self, row, column):
        ...
        row_offset = 2 if row in {0, self.maze.height - 1} else 1
        column_offset = 2 if column in {0, self.maze.width - 1} else 1
        x0 = self.CELL_WIDTH * column + self.BORDER_OFFSET + column_offset
        y0 = self.CELL_HEIGHT * row + self.BORDER_OFFSET + row_offset
        x1 = x0 + self.CELL_WIDTH - (2 if column == self.maze.width - 1 else 1) * column_offset
        y1 = y0 + self.CELL_HEIGHT - (2 if row == self.maze.height - 1 else 1) * row_offset
    ...
    def tick(self):
        self.stats['text'] = (f'Maze Size: {self.maze.width} x {self.maze.height}         '
